...
class PowerQueryTab(QWidget):
    ...
    def init_ui(self):
        ...
        right_layout.addWidget(QLabel("M Query"))

        self.m_query_editor = QCodeEditor(language="m")
        self.m_query_editor.setFont(code_editor_font())
        self.m_query_editor.setEnabled(False)
        try:
            space_w = self.m_query_editor.fontMetrics().horizontalAdvance(" ")
            self.m_query_editor.setTabStopDistance(space_w * 4)
        except Exception:
            pass
        self.m_query_editor.textChanged.connect(self.on_m_query_changed)
        right_layout.addWidget(self.m_query_editor)

        form_layout = QFormLayout()
        form_layout.setContentsMargins(0, 8, 0, 0)
        form_layout.setSpacing(6)

        self.import_mode_edit = QLineEdit()
        self.import_mode_edit.setEnabled(False)
        self.import_mode_edit.textEdited.connect(self.on_import_mode_changed)
        form_layout.addRow("Import mode", self.import_mode_edit)

        self.query_group_edit = QLineEdit()
        self.query_group_edit.setEnabled(False)
        self.query_group_edit.textEdited.connect(self.on_query_group_changed)
        form_layout.addRow("Query group", self.query_group_edit)
    ...
    def load_tables(self):
        ...
        try:
            ...
            for filename in os.listdir(tables_dir):
                ...
                #         return _strip_any_fence(result) if result else None
                ...
                #     result = _strip_any_fence(result)
                ...
                tables_data[table_name] = {
                    "columns": columns,
                    "import_mode": mode, # mode_match.group(1).strip() if mode_match else "",
                    "query_group": query_group, # query_group_match.group(1).strip() if query_group_match else "",
                    "m_code": extract_m_code(tmdl_data) # expression_match.group(1) if expression_match else "",
                }
    ...
    def display_table_details(self, table_name: Optional[str]):
        ...
        if not table_name or table_name not in self.tables_data:
            self.m_query_editor.setPlainText("")
            self.m_query_editor.setEnabled(False)
            self.import_mode_edit.setText("")
            self.import_mode_edit.setEnabled(False)
            self.query_group_edit.setText("")
            self.query_group_edit.setEnabled(False)
            self.ignore_editor_changes = False
            return
        ...
        self.m_query_editor.setEnabled(True)
        self.m_query_editor.setPlainText(table_info.get("m_code") or "")

        self.import_mode_edit.setEnabled(True)
        self.import_mode_edit.setText(table_info.get("import_mode") or "")

        self.query_group_edit.setEnabled(True)
        self.query_group_edit.setText(table_info.get("query_group") or "")

        self.ignore_editor_changes = False

    def on_m_query_changed(self):
        """Persist M query changes to in-memory state."""
        if self.ignore_editor_changes or not self.current_table:
            return
        self.tables_data[self.current_table]["m_code"] = self.m_query_editor.toPlainText()

    def on_import_mode_changed(self, text: str):
        """Persist import mode changes to in-memory state."""
        if self.ignore_editor_changes or not self.current_table:
            return
        self.tables_data[self.current_table]["import_mode"] = text

    def on_query_group_changed(self, text: str):
        """Persist query group changes to in-memory state."""
        if self.ignore_editor_changes or not self.current_table:
            return
        self.tables_data[self.current_table]["query_group"] = text
    ...
    def clear_details(self):
        ...
        self.ignore_editor_changes = True
        self.m_query_editor.clear()
        self.m_query_editor.setEnabled(False)
        self.import_mode_edit.clear()
        self.import_mode_edit.setEnabled(False)
        self.query_group_edit.clear()
        self.query_group_edit.setEnabled(False)
        self.ignore_editor_changes = False
